import AI.AIQAAgent;
import AI.AIQAAgentResult;
import AI.AITestCase;
import AI.MockAIClient;

import org.testng.Assert;
import org.testng.annotations.Test;

import java.util.List;

public class AIQAAgentRequirementTest {

    @Test
    public void testRequirementToPrioritizedTestCases()
            throws Exception {

        System.out.println();
        System.out.println("======================================");
        System.out.println("   REQUIREMENT TO TEST CASE TEST");
        System.out.println("======================================");

        // Step 1: Create AI QA Agent
        AIQAAgent agent =
                new AIQAAgent(
                        new MockAIClient()
                );

        // Step 2: Provide a real requirement
        String requirement =
                "The user should be able to log in "
                        + "with a valid username and password.";

        // Step 3: Process the requirement
        AIQAAgentResult result =
                agent.processRequirement(
                        requirement
                );

        // Step 4: Verify validation passed
        Assert.assertTrue(
                result.isPassed(),
                "Requirement processing should pass."
        );

        // Step 5: Verify there are no errors
        Assert.assertEquals(
                result.getErrorCount(),
                0,
                "There should be no validation errors."
        );

        // Step 6: Get prioritised test cases
        List<AITestCase> testCases =
                result.getTestCases();

        // Step 7: Verify number of test cases
        Assert.assertEquals(
                testCases.size(),
                4,
                "There should be 4 test cases."
        );

        // Step 8: Verify prioritised order
        Assert.assertEquals(
                testCases.get(0).test_case_id,
                "TC004",
                "Security test case should be first."
        );

        Assert.assertEquals(
                testCases.get(1).test_case_id,
                "TC002",
                "Negative test case should be second."
        );

        Assert.assertEquals(
                testCases.get(2).test_case_id,
                "TC003",
                "Boundary test case should be third."
        );

        Assert.assertEquals(
                testCases.get(3).test_case_id,
                "TC001",
                "Positive test case should be fourth."
        );

        System.out.println();
        System.out.println(
                "✅ Requirement workflow test passed."
        );
    }
}